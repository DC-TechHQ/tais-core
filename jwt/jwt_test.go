...
var testCfg = jwt.Config{Secret: "test-secret-key-32-bytes-long!!", TTLSeconds: 3600}

func TestSign_And_Parse(t *testing.T) {
	claims := &jwt.Claims{
		Sub:   42,
		Type:  jwt.TypeStaff,
		IpNet: "10.200.1",
	...
	token, err := jwt.Sign(claims, testCfg)
	if err != nil {
		t.Fatalf("Sign: %v", err)
	}
	if token == "" {
...
func TestParse_WrongSecret(t *testing.T) {
	...
	token, _ := jwt.Sign(claims, testCfg)
